...
Function TestTLSSettings
{
  ...
  if ($tlsKeyExists -eq $false) {
    Write-Host("Registry Key does not exist:" + $regKey);
    Write-Host("TLS 1.2 is not explicitly enabled. Please enable it.");
    return;
  }
  ...
  if ($tlsServerKeyExists -eq $false) {
    Write-Host("TLS is either not enabled or disabled by default:" + $regKey);
    Write-Host("TLS 1.2 is not explicitly enabled. Please enable it.");
    return;
  }
