...
$users = Get-ADGroupMember -Identity G_O365-Sync | Where-Object { $_.SamAccountName -ne "ÖffentlicherOrdner" } | ForEach-Object {Get-ADUser $_.SamAccountName | Select-Object userPrincipalName }
...
Foreach ($user in $users)
	{
	Try {
		New-MoveRequest -Erroraction Stop -Identity $user.userPrincipalName -Remote -RemoteHostName $endpoint.RemoteServer -TargetDeliveryDomain 'stwb.mail.onmicrosoft.com' -RemoteCredential $opcred -SuspendWhenReadyToComplete:$true
